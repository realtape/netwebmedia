...
**2026-05-29 browser route also blocked (Business Suite, NWM Chrome):** Opening business.facebook.com/latest/home for the NetWebmedia portfolio shows a **"Verification needed — verify your account before you can proceed"** modal (Verify account button) gating the whole account — likely lingering from the 5/25 restriction. Do NOT click Verify (identity/auth = Carlos only). The asset tree confirms IG is STILL not linked: **NetWebmedia = "Facebook Page" only** (sibling brand SpiritsBuy shows "Facebook Page, Instagram account"). The portfolio is "spiritsbuy"; admins = "IG" + Carlos Martinez; FB followers 33. Net: neither API nor browser can post until Carlos (1) clears the verification wall, (2) regenerates FB_PAGE_TOKEN + redeploys, (3) links @netwebmedia (Professional acct) to the Page.
