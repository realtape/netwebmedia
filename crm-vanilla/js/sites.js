...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    CRM_APP.buildHeader(CRM_APP.t('nav.sites'), '<button class="btn btn-primary" id="newSiteBtn">' + CRM_APP.ICONS.plus + ' ' + L.newSite + '</button>');
    var newBtn = document.getElementById("newSiteBtn");
    if (newBtn) newBtn.addEventListener("click", function () { notifyBeta('new-site'); });
  ...
  function notifyBeta(action) {
    var isEs = (window.CRM_APP && CRM_APP.getLang && CRM_APP.getLang() === 'es');
    var msg = isEs
      ? 'El módulo de Sitios está en beta (lanza Q3 2026). Mientras tanto, contáctanos para esta solicitud.'
      : 'The Sites module is in beta (ships Q3 2026). Contact us for early-access requests in the meantime.';
    if (confirm(msg + '\n\n' + (isEs ? '¿Abrir formulario de contacto?' : 'Open the contact form?'))) {
      window.location.href = '/contact.html?topic=sites-beta&action=' + encodeURIComponent(action);
    }
  }
