...
- **Canonical phone number — `+1 (760) 334-8731` (activated 2026-05-13).** Same line for voice and WhatsApp click-to-chat. Formats: display `+1 (760) 334-8731`, `tel:+17603348731`, `https://wa.me/17603348731`, schema.org `+1-760-334-8731`, E.164 `+17603348731`. Direct `wa.me/17603348731` links live on `whatsapp.html` and `contact.html` only — the routing-through-`/whatsapp.html` rule above still applies for every other surface. Outbound WhatsApp broadcasts (`wa_flush.php` → Meta Cloud API, gated by `WA_PHONE_ID`) remain pending Meta business verification (target June 2026); inbound click-to-chat on the new number works today. The public-chatbot KB (`api-php/lib/knowledge-base.php`) used to say "we don't offer phone support" — that was rewritten to "async-first, but voice + WhatsApp on the same number when real-time is the right fit"; do not restore the old line.
