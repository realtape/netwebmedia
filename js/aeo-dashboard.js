// AEO Pulse — interactive hero dashboard (Claude Design port, 2026-05-19)
// Mounts into #dashboard-root. Requires React + ReactDOM 18 loaded first.
// All data below is a static demo (seeded RNG + fixed feed pool) — no live API.

(function () {
  if (!window.React || !window.ReactDOM) {
    // React not loaded yet; retry shortly. Loaded as defer scripts so this is rare.
  ...
  function Feed({ pool, paused }) {
    const [items, setItems] = useState(() =>
      pool.slice(0, 5).map((it, i) => Object.assign({}, it, { t: i === 0 ? 'now' : `${i * 17}s`, id: i }))
    );
    ...
    useEffect(() => {
      ...
      const id = setInterval(() => {
        setItems(prev => {
          ...
          const fresh = Object.assign({}, next, { t: 'now', id: idRef.current++ });
          const aged = prev.map(p => Object.assign({}, p, { t: ageUp(p.t) }));
  ...
  function Donut({ series, hidden, onToggle }) {
    const totals = series.map(s => Object.assign({}, s, {
  ...
  function Dashboard() {
    ...
    function toggleEngine(k) {
      setHidden(prev => Object.assign({}, prev, { [k]: !prev[k] }));
    }
      ...
      tab === 'citations' && h('div', { className: 'pane' },
        ...
        h('div', { className: 'cite-grid' },
          ENGINES.map(e => {
            ...
            return h('div', { className: 'cite-card', key: e.key, style: { borderColor: hidden[e.key] ? 'var(--hc-line)' : `${e.color}55`, opacity: hidden[e.key] ? 0.4 : 1 } },
  ...
  if (root) ReactDOM.createRoot(root).render(h(Dashboard));
})();
