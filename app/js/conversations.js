...
(function () {
  ...
  function renderConversationList() {
    ...
    if (convos.length === 0) {
      container.innerHTML = '<div class="empty-conv">No conversations found</div>';
      return;
    }
    ...
    for (var j = 0; j < items.length; j++) {
      items[j].addEventListener("click", function () {
        selectConversation(parseInt(this.getAttribute("data-id"), 10));
      });
    }
  }
  ...
  function renderChatPanel() {
    ...
    header.innerHTML =
      ...
      '<div>' +
      '<div class="chat-header-name">' + c.contact + '</div>' +
      '<div class="chat-header-channel">' + CRM_APP.channelIcon(c.channel) + ' ' +
    ...
    for (var i = 0; i < c.messages.length; i++) {
      ...
      if (!isYou) html += '<div class="msg-avatar">' + c.avatar + '</div>';
  ...
  function sendMessage() {
    ...
    if (!text) return;
    activeConversation.messages.push({ from: "you", text: text, time: "Just now" });
    input.value = "";
    renderChatPanel();
  }
