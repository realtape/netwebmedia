...
function route_public_chat() {
  ...
  if (!empty($r['error'])) {
    ...
    // TEMP diagnostic: when caller passes X-Debug-Token, surface the raw Claude error.
    // Remove after diagnosing. Token intentionally non-obvious; rotate if it leaks.
    $dbg = $_SERVER['HTTP_X_DEBUG_TOKEN'] ?? '';
    $dbgPayload = ($dbg === 'nwm-diag-2026-04-21') ? [
      'debug_error' => $r['error'] ?? null,
      'debug_code'  => $r['code']  ?? null,
      'debug_raw'   => $r['raw']   ?? null,
      'debug_key_prefix' => function_exists('ai_anthropic_key')
        ? (substr((string)ai_anthropic_key(), 0, 10) . '…(' . strlen((string)ai_anthropic_key()) . ' chars)')
        : 'n/a',
    ] : [];
