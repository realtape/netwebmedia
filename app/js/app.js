...
/* Inject api-client.js if a CRM HTML page didn't load it explicitly. */
(function () {
...
(function () {
  ...
  /* ── Navigation items ── */
  var NAV_ITEMS = [
    { id: "dashboard",         label: "Dashboard",          i18n: "crm_dashboard",           icon: "dashboard",     href: "index.html" },
    { id: "conversations",     label: "Conversations",      i18n: "crm_conversations",       icon: "conversations", href: "conversations.html" },
    { id: "calendars",         label: "Calendars",          i18n: "crm_calendars",           icon: "calendar",      href: "calendar.html" },
    { id: "contacts",          label: "Contacts",           i18n: "crm_contacts",            icon: "contacts",      href: "contacts.html" },
    { id: "sales-pipeline",    label: "Sales Pipeline",     i18n: "crm_sales_pipeline",      icon: "pipeline",      href: "sales-pipeline.html" },
    { id: "pipeline",          label: "Opportunities",      i18n: "crm_opportunities",       icon: "pipeline",      href: "pipeline.html" },
    { id: "payments",          label: "Payments",           i18n: "crm_payments",            icon: "payments",      href: "payments.html" },
    { id: "marketing-pipeline",label: "Marketing Pipeline", i18n: "crm_marketing_pipeline",  icon: "reporting",     href: "marketing-pipeline.html" },
    { id: "marketing",         label: "Marketing",          i18n: "crm_marketing",           icon: "marketing",     href: "marketing.html" },
    { id: "automation",        label: "Automation",         i18n: "crm_automation",          icon: "automation",    href: "automation.html" },
    { id: "sites",             label: "Sites",              i18n: "crm_sites",               icon: "sites",         href: "sites.html" },
    { id: "reputation",        label: "Reputation",         i18n: "crm_reputation",          icon: "reputation",    href: "reputation.html" },
    { id: "reporting",         label: "Reporting",          i18n: "crm_reporting",           icon: "reporting",     href: "reporting.html" },
    { id: "documents",         label: "Documents",          i18n: "crm_documents",           icon: "documents",     href: "documents.html" },
    { id: "courses",           label: "Courses",            i18n: "crm_courses",             icon: "courses",       href: "courses.html" },
    { id: "social",            label: "Social Planner",     i18n: "crm_social_planner",      icon: "social",        href: "social.html" },
    { id: "settings",          label: "Settings",           i18n: "crm_settings",            icon: "settings",      href: "settings.html" }
  ];
  function crmLbl(item){ return (window.NWMi18n && window.NWMi18n.t) ? (window.NWMi18n.t(item.i18n) || item.label) : item.label; }
  ...
  function buildSidebar() {
    ...
    for (var i = 0; i < NAV_ITEMS.length; i++) {
      ...
      var lbl = crmLbl(item);
      html += '<a href="' + item.href + '" class="nav-item' + isActive + '" data-i18n="' + item.i18n + '" title="' + lbl + '">';
      html += '<span class="nav-icon">' + ICONS[item.icon] + '</span>';
      html += '<span class="nav-label">' + lbl + '</span>';
      html += '</a>';
    }
    html += '</nav>';

    html += '<div class="sidebar-footer">';
    html += '<a href="../cms/index.html" class="service-switch service-switch-cms">';
    html += '<div class="switch-dot switch-dot-cms"></div>';
    html += '<div class="switch-body"><div class="switch-label" data-i18n="switch_to">Switch to</div><div class="switch-name" data-i18n="switch_cms">NetWeb CMS</div></div>';
    html += '</a>';
    html += '<div id="sidebar-lang" style="margin:10px auto;display:flex;justify-content:center;"></div>';
    ...
    // i18n: apply translations to sidebar and mount language toggle
    if (window.NWMi18n) {
      window.NWMi18n.apply(sidebar);
      var mount = document.getElementById('sidebar-lang');
      if (mount) window.NWMi18n.injectToggle(mount);
    }
    if (!buildSidebar._langBound) {
      buildSidebar._langBound = true;
  ...
  function getLoggedInUser() {
    // 1) Real logged-in user from the API (localStorage key: nwm_user)
    try {
      var apiRaw = localStorage.getItem('nwm_user');
      if (apiRaw) {
        var u = JSON.parse(apiRaw);
        if (u && u.email) {
          // Auto-cleanup: real user present → wipe legacy demo key
          try { localStorage.removeItem('crm_demo_user'); } catch (_) {}
          return {
            name: u.name || u.email,
            email: u.email,
            type: u.role === 'admin' ? 'admin' : (u.role || 'user'),
            id: u.id,
            org_id: u.org_id
          };
        }
