        ...
        case 'push_for_approval':
            try {
                ...
                if (empty($ctx['push_notified_at'])) {
                    ...
                    // Phase 2: re-enter after wait — check approval status
                    ...
                    if ($draftStatus === 'approved') {
                        ...
                        // Timed out while still pending — holding_reply takes over next.
 ...
 * flip the draft status to 'sent'. Safe to call from both push_for_approval
 * Phase 2 and the messages.php operator handler (first-write-wins race guard
 * is the caller's UPDATE ... WHERE status='pending_approval' rowCount check).
