...
yt_upload_v9.py
===============
Parameterized YouTube upload script. Usage:
  python yt_upload_v9.py <full_path_to_mp4>

ROOT CAUSE (solved in v8): Claude Electron windows (hwnd=132606, 13174642)
were maximized full-screen ON TOP of Chrome, intercepting all mouse clicks.
Fix: minimize them first, then do upload flow.

Steps:
  1. Minimize Claude Electron windows
  2. Force Chrome (hwnd=2820290) to front
  3. Navigate to YouTube Studio via Ctrl+L
  4. Click Create -> Upload videos -> Select files
  5. Paste file path into dialog (clipboard fallback)
  6. Restore Claude windows
"""

import ctypes, ctypes.wintypes, time, os, sys, subprocess

if len(sys.argv) < 2:
    print("Usage: python yt_upload_v9.py <path_to_mp4>")
    sys.exit(1)

FILE_PATH    = sys.argv[1]
LOG_PATH     = r"C:\Users\Usuario\Desktop\NetWebMedia\hyperframes\nwm-reels\yt_upload_v9.log"
CHROME_HWND  = 2820290
CLAUDE_HWND1 = 132606
...
if "Studio" not in title_now and "YouTube" not in title_now:
    log("  Not on Studio yet - cycling tabs...")
    for i in range(15):
        ctrl(0x09)
        time.sleep(0.4)
        t=get_title(hwnd)
