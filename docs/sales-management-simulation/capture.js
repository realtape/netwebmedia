// Sales Management Simulation — Sarah Test buys CMO Standard.
...
//   5. Proposal Sent     — CMO Standard quote attached
...
function dealAtStage(stageIdx) {
  ...
  return {
    id: 9001,
    contact_id: BUYER.id,
    title: 'Test Law Group — CMO Standard',
    company: BUYER.company,
    contact_name: BUYER.name,
    contact_email: BUYER.email,
    stage: stage.name,
    stage_id: stage.id,
    value: stageIdx === 0 ? 1490 : (stageIdx <= 3 ? 1490 : 17880),  // monthly → annual after discovery
    amount: 1490,                  // CMO Standard monthly
    setup_fee: 499,
    annual_value: 17880,           // 12 * 1490
...
(async () => {
    ...
    const route = (u) => {
      ...
      if (/r=contacts/.test(u) || /\/contacts\b/.test(u)) {
        return [{
          ...
          source: 'aeo_brief_001', tags: ['cmo-standard-buyer','law-firm','high-intent'],
          owner_id: 1, owner_name: 'Carlos Martinez',
          last_contacted_at: '2026-05-11T09:42:00Z',
          lifetime_value: currentDeal && currentDeal._won_or_active ? 17880 : 0,
      ...
      if (/dashboard|overview|stats/i.test(u)) {
        ...
        return {
          ok: true,
          revenue: { current: won ? 1989 : 0, previous: 0, delta_pct: 0 },
      ...
      if (/r=documents|\/documents\b/.test(u)) {
        return [
          { id: 1, name: 'CMO Standard Proposal — Test Law Group.pdf', kind: 'proposal', deal_id: 9001, contact_id: buyer.id, sent_at: '2026-05-09', status: 'viewed', size_kb: 412 },
      ...
      if (/r=payments|\/payments\b/.test(u)) {
        return [
          { id: 'inv_001', number: 'NWM-2026-0042', deal_id: 9001, contact: buyer.name, company: buyer.company, line_items: 'Setup fee + Month 1', amount: 1989, status: 'paid', paid_at: '2026-05-11', method: 'Stripe' },
          { id: 'sub_001', number: 'SUB-NWM-0042', deal_id: 9001, contact: buyer.name, company: buyer.company, line_items: 'CMO Standard monthly', amount: 1490, status: 'active', next_charge_at: '2026-06-11', method: 'Stripe' },
        ];
      }
  ...
  const ACTIVITY_BY_STAGE = [
    ...
    [
      ...
      { at: '2026-05-04 14:48', type: 'email',    who: 'Sarah',   text: 'Replied to welcome-1 — "Yes, looking for an audit + recommendation on CMO Standard"' },
    ...
    [
      ...
      { at: '2026-05-07 10:48', type: 'note',    who: 'Carlos',  text: 'Sarah is 4-attorney PI firm · 12 leads/mo, ~18% converting to retainer · Wants CMO Standard with focus on intake automation' },
    ],
    [
      { at: '2026-05-09 09:14', type: 'document', who: 'Carlos', text: 'Sent · CMO Standard Proposal — Test Law Group.pdf (4 pages)' },
    ...
    [
      ...
      { at: '2026-05-11 09:42', type: 'payment',  who: 'system', text: 'Stripe charge · $1,989 (setup $499 + month 1 $1,490) · paid' },
    ],
    [
