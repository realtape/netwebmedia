...
- **CRM** = repo dir `crm-vanilla/`, public URL `netwebmedia.com/crm/` (flipped 2026-05-28; /crm-vanilla/ 301s) → DB `webmed6_crm`. Auth = **PHP session** (`$_SESSION['nwm_uid']`) set by logging in at `netwebmedia.com/crm/login.html` (`?r=auth`). The real Chile/USA leads live here.
...
- To send from the CRM, Carlos must be logged in at `/crm/login.html` (sets the session cookie my same-origin fetch then carries).

**Self-service password reset (shipped 2026-05-25, commit 203f50568):**
- CRM login "Forgot password?" → `POST /crm/api/?r=password_reset&action=request {email}` → emails a tokenized link → `reset-password.html?token=...` → `action=confirm {token,password}`.
