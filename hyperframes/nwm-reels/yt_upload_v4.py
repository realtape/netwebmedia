...
yt_upload_v4.py
===============
Target the main Chrome window (hwnd=2820290) which already has Studio tabs.
1. Force focus to that window
2. Click the Studio tab (visible in tab bar at ~x=980 or 1130, y=14)
3. Wait for Studio to become active
4. Click Create -> Upload videos -> Select files
5. Fill file dialog via WM_SETTEXT

Screen: 1920x1080, Chrome maximized at (-8,-8,1936,1048)
Tab bar y = wy + 25 = -8 + 25 = 17
"""
...
def force_focus(hwnd):
    ...
    log(f"  force_focus hwnd={hwnd}: ok={new_fg==hwnd} new_fg={new_fg}")
    return new_fg==hwnd
...
log(f"Target hwnd={hwnd} title='{title}'")
if not user32.IsWindowVisible(hwnd):
    log("Target window not visible - aborting"); sys.exit(1)
...
TAB_Y = wy + 25  # tab bar center y in screen coords (=-8+25=17)
log(f"Tab bar y = {TAB_Y}")

# ── Click Studio tabs in the tab bar ──────────────────────────────────────────
# From screenshot: Studio tabs appear at approximately x=980 and x=1130, y=14
# Scan from right to left looking for the Studio tab
log("Scanning for Studio tab...")
studio_tab_x_candidates = [1130, 1080, 1030, 980, 930, 880, 1180, 1230]

for tx in studio_tab_x_candidates:
    log(f"  Clicking tab at ({tx},{TAB_Y})")
    click(tx, TAB_Y, delay=0.6)
    t=get_title(hwnd)
    log(f"    title='{t}'")
    if "YouTube Studio" in t or "Studio" in t:
...
# ── If not on Studio, navigate via address bar ─────────────────────────────────
if "Studio" not in title_now and "YouTube" not in title_now:
    log("Not on Studio - navigating via address bar...")
    set_clip(STUDIO_URL)
    force_focus(hwnd); time.sleep(0.3)
    addr_y = wy + 60
    addr_x = wx + ww // 2
    click(addr_x, addr_y, delay=0.3)
    ctrl(VK_A); time.sleep(0.1)
    ctrl(VK_V); time.sleep(0.2)
    vkey(VK_RETURN)
    log("Navigating to Studio, waiting 8s...")
    time.sleep(8)
    title_now=get_title(hwnd)
    log(f"Title: '{title_now}'")

# ── Re-focus and get window rect for upload flow ───────────────────────────────
log("Re-focusing for upload flow...")
