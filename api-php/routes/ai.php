...
function ai_call_claude($systemPrompt, $userMessage, $history = [], $model = 'claude-3-5-sonnet-20241022') {
...
function route_ai($parts, $method) {
  ...
  if ($sub === 'chat' && $method === 'POST') {
    ...
    $r = ai_call_claude($data['system_prompt'] ?? '', $b['message'], $history, $data['model'] ?? 'claude-3-5-sonnet-20241022');
