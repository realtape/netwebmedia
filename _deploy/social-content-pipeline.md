...
| **WhatsApp Business** | 3 broadcasts | Public click-to-chat is now **+1 (442) 385-4585** (`wa.me/14423854585`, verified 2026-05-25) — but on the WhatsApp Business **app**, so no Cloud-API outbound broadcasts | **REPLACED.** Outbound WA broadcasts stay converted to email follow-ups in the warm-prospect sequence. Re-introduce outbound WA only with a Cloud-API WABA (the App number can't broadcast; infra wired: `WA_META_TOKEN`, `WA_PHONE_ID`, `WA_META_APP_SECRET`). |
