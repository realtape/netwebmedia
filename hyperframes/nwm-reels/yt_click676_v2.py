...
yt_click676_v2.py
=================
Same as yt_click676.py but first cycles Chrome tabs using Ctrl+Tab
until the window title contains 'YouTube Studio', ensuring the upload
wizard tab is active before clicking Select files at y=676.

Usage: python yt_click676_v2.py <path_to_mp4>
"""
import ctypes, ctypes.wintypes, time, sys, subprocess

if len(sys.argv) < 2:
    print("Usage: python yt_click676_v2.py <path_to_mp4>"); sys.exit(1)
...
def ctrl_tab():
    si(INPUT(INPUT_KEYBOARD,_U(ki=KI(VK_CONTROL,0,0,0,None))),
       INPUT(INPUT_KEYBOARD,_U(ki=KI(VK_TAB,0,0,0,None))),
       INPUT(INPUT_KEYBOARD,_U(ki=KI(VK_TAB,0,KEYEVENTF_KEYUP,0,None))),
       INPUT(INPUT_KEYBOARD,_U(ki=KI(VK_CONTROL,0,KEYEVENTF_KEYUP,0,None)))); time.sleep(0.3)
...
# Step 3: Focus Chrome
hwnd = CHROME_HWND
log(f"Focusing Chrome hwnd={hwnd}")
force_focus(hwnd)

# Step 4: Cycle Chrome tabs until YouTube Studio is active
log(f"Current Chrome title: '{get_title(hwnd)}'")
log("Switching Chrome tabs to find YouTube Studio...")
found_yt = False
for attempt in range(20):
    title = get_title(hwnd)
    log(f"  Tab {attempt}: '{title}'")
    if 'YouTube Studio' in title or 'youtube.com/channel' in title.lower():
        log(f"  Found YouTube Studio tab!")
        found_yt = True
        break
...
if not found_yt:
    log("WARNING: Could not find YouTube Studio tab, proceeding anyway")

# Brief pause to let the tab settle
time.sleep(0.5)
log(f"Active Chrome title now: '{get_title(hwnd)}'")

# Step 5: Click Select files at y=676
