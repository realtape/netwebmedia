...
**How to apply:**
- **Computer 2 ([[user_computers]]) still has the OLD diverged `main`.** It MUST re-clone or `git fetch origin && git reset --hard origin/main` BEFORE its daemon runs again, or it will re-pollute `origin/main` and undo the rewrite. Flag this until Carlos confirms Computer 2 is re-cloned.
