...
- **`/whatsapp.html` is the canonical "contact us via WhatsApp" landing**, NOT a direct `wa.me/...` link. The Twilio sandbox `wa.me/14155238886` is dead — every public CTA across 28 files was swept (2026-05-01) to point at `/whatsapp.html`, which has intent-aware copy via `?topic=` and offers email + WhatsApp-list + contact-form fallbacks. Public CTAs across the site continue to route through `/whatsapp.html` — do NOT spray new `wa.me/` links across pages.
- **Phone numbers (split 2026-05-25 — voice and WhatsApp are now SEPARATE lines).** Voice: `+1 (760) 334-8731` (Sonetel) — `tel:+17603348731`, schema.org `+1-760-334-8731`, E.164 `+17603348731`. WhatsApp: `+1 (619) 738-6150` (Zadarma, registered on the WhatsApp Business **app**) — `https://wa.me/16197386150`, E.164 `+16197386150`. Direct `wa.me/16197386150` links live on `whatsapp.html` and `contact.html` only — the routing-through-`/whatsapp.html` rule above still applies for every other surface. **Because WhatsApp is on the Business App (not Cloud API), outbound broadcasts via `wa_flush.php` (Meta Cloud API, `WA_PHONE_ID`) are NOT available on the 619 number — the App can't do Cloud API, and the number is locked to the App.** The old Sonetel/Meta WABA verification is now decoupled from the public WhatsApp number. The public-chatbot KB (`api-php/lib/knowledge-base.php`) and `kb.json` split the two numbers (voice 760 / WhatsApp 619) — do not collapse them back to "same number," and don't restore the old "we don't offer phone support" line.
